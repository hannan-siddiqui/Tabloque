...
export const NoteCardColumn: React.FC<NoteCardColumnProps> = ({
  board,
  notes,
  ...
  const handleResizeMouseDown = (e: React.MouseEvent) => {
    ...
    const startX = e.clientX;
    const startY = e.clientY;
    const startWidth = currentDimensions.width || board.width || 340;
    const startHeight = currentDimensions.height || board.height || 380;
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      ...
      const newWidth = Math.max(240, Math.min(950, Math.round(startWidth + deltaX)));
  ...
  return (
    ...
    >
      ...
      {/* Notes List (responsive multi-column when widened) */}
      <div
        className={`flex-1 overflow-y-auto custom-scrollbar pr-1 pb-1 ${
          isMultiColumn
            ? 'grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-2.5 content-start'
            : 'space-y-2 py-1'
        }`}
      >
        {notes.length === 0 ? (
          <div className="py-8 text-center text-slate-400 text-xs flex flex-col items-center gap-2">
            <StickyNote className="w-7 h-7 text-slate-600 opacity-50 stroke-[1.5]" />
            <p>No notes in this card yet</p>
            <button
              type="button"
              onClick={() => onAddNote(board.id)}
              className="text-[11px] font-semibold text-[var(--theme-accent,#22c55e)] hover:underline cursor-pointer"
            >
              + Add first note
            </button>
          </div>
